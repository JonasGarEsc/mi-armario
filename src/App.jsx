...
export default function App() {
  ...
  async function limpiarStorage() {
    if (!confirm('Esta acción detectará y eliminará imágenes en la nube que no estén asociadas a ninguna prenda. ¿Ejecutar limpieza?')) return
    ...
    if (archivosBorrables.length > 0) {
      ...
      vibrar([50, 50, 50])
      alert(`Limpieza optimizada: ${archivosBorrables.length} archivos residuales eliminados.`)
    } else {
      alert('Sistema limpio. No hay archivos residuales.')
    }
  }
      ...
      <div className="min-h-screen bg-linear-to-br from-rose-50 via-white to-teal-50 in-[.modo-oscuro_&]:from-[#13111C] in-[.modo-oscuro_&]:via-[#1F1D2B] in-[.modo-oscuro_&]:to-[#251F31] text-slate-800 in-[.modo-oscuro_&]:text-[#E0D8F0] flex flex-col w-full overflow-x-hidden transition-colors duration-500">
        <header className="bg-white/70 backdrop-blur-md in-[.modo-oscuro_&]:bg-[#1A1825]/70 shadow-sm border-b border-rose-100/50 in-[.modo-oscuro_&]:border-[#322F44]/50 p-3 md:p-4 flex justify-between items-center w-full sticky top-0 z-10">
          ...
          <div className="flex gap-2">
            <button onClick={limpiarStorage} className="text-xs font-bold bg-white/50 in-[.modo-oscuro_&]:bg-[#2A273F]/50 border border-rose-100 in-[.modo-oscuro_&]:border-[#433D60] px-3 py-1.5 rounded-full cursor-pointer hover:bg-white in-[.modo-oscuro_&]:hover:bg-[#34304D] shadow-sm active:scale-95 transition-transform" title="Limpiar datos inútiles">
              🧹
            </button>
        ...
        <main {...swipeNavegacion} className="flex-1 w-full p-2 md:p-6 lg:p-8 touch-pan-y">
        ...
        {modalActivo && (
          <div className="fixed inset-0 bg-rose-900/30 in-[.modo-oscuro_&]:bg-black/60 flex items-end sm:items-center justify-center z-50 p-0 sm:p-4 backdrop-blur-sm transition-opacity">
            <div {...swipeModal} className="bg-white/95 in-[.modo-oscuro_&]:bg-[#1F1D2B]/95 border-t sm:border border-white/50 in-[.modo-oscuro_&]:border-[#322F44] rounded-t-4xl sm:rounded-3xl shadow-2xl shadow-rose-900/30 w-full max-w-2xl p-5 sm:p-8 relative max-h-[90vh] overflow-y-auto animate-slide-up touch-pan-x">
              <div className="w-12 h-1.5 bg-slate-300 in-[.modo-oscuro_&]:bg-[#433D60] rounded-full mx-auto mb-5 sm:hidden opacity-70"></div>
