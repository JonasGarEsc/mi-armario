  ...
  function solicitarEliminacionMaleta(id, e) {
    ...
    setDialogoConfirmacion({
      ...
      accion: async () => {
        ...
        setDialogoConfirmacion(null); vibrar([50, 50])
      }
    })
  }
    ...
    <div className="flex flex-col h-full w-full animate-fade-in-up relative">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 md:mb-8 gap-3 md:gap-4 pb-4 md:pb-6 w-full">
        {maletaActiva ? (
          ...
          <h3 className="font-bold text-xl md:text-2xl text-transparent bg-clip-text bg-linear-to-r from-teal-600 to-rose-400 in-[.modo-oscuro_&]:from-[#A394D6] in-[.modo-oscuro_&]:to-[#C2A3FF] hidden sm:block">Mis maletas</h3>
        )}

        <div className="flex w-full sm:w-auto gap-2 md:gap-4">
          <input type="text" autoComplete="off" placeholder={!maletaActiva ? "Buscar maleta..." : "Buscar conjunto..."} value={busqueda} onChange={(e) => setBusqueda(e.target.value)} className="flex-1 border border-rose-200/50 in-[.modo-oscuro_&]:border-[#433D60] bg-white/80 in-[.modo-oscuro_&]:bg-[#2A273F]/80 text-slate-800 in-[.modo-oscuro_&]:text-[#E0D8F0] rounded-xl p-2.5 md:p-3 px-4 text-xs md:text-sm focus:ring-4 focus:ring-teal-400/30 in-[.modo-oscuro_&]:focus:ring-[#A394D6]/30 outline-none shadow-sm transition-all" />
          {!maletaActiva && (
            <button onClick={onCrearMaleta} className="bg-linear-to-r from-teal-400 to-emerald-400 in-[.modo-oscuro_&]:from-[#7E67C9] in-[.modo-oscuro_&]:to-[#9985D8] text-white px-4 md:px-6 py-2.5 md:py-3 rounded-xl md:rounded-2xl font-bold shadow-lg cursor-pointer hover:scale-105 active:scale-95 text-xs md:text-base whitespace-nowrap">+ Preparar</button>
          )}
        </div>
      </div>
      ...
      {maletaActiva && (
        <div className="flex flex-col items-center mb-6 animate-pop-in">
          <h2 className="text-xl md:text-3xl font-extrabold text-slate-800 in-[.modo-oscuro_&]:text-[#E0D8F0] tracking-tight">{maletaActiva.nombre}</h2>
          {clima && (
            <span className="text-xs md:text-sm font-bold bg-white/80 in-[.modo-oscuro_&]:bg-[#2A273F] px-3 py-1 mt-2 rounded-full shadow-sm text-slate-600 in-[.modo-oscuro_&]:text-[#A394D6] border border-rose-100 in-[.modo-oscuro_&]:border-[#433D60]">
      ...
      {cargando ? (
        <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-2 md:gap-4 px-1">
           {[...Array(6)].map((_, i) => <div key={i} className="rounded-xl h-32 md:h-64 bg-white/50 in-[.modo-oscuro_&]:bg-[#2A273F]/50 animate-shimmer"></div>)}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 px-2 w-full">
          {itemsFiltrados.map((conj, i) => (
            <div key={conj.id} className="relative border border-white/50 in-[.modo-oscuro_&]:border-[#322F44]/50 p-6 rounded-4xl bg-white/50 in-[.modo-oscuro_&]:bg-[#1F1D2B]/50 backdrop-blur-md shadow-lg group animate-fade-in-up w-full flex flex-col">
              ...
              <div className="absolute top-5 right-5 flex gap-2 z-50 opacity-100 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity">
                <button onClick={() => { setConjuntoADuplicar(conj); vibrar(30); }} className="bg-white in-[.modo-oscuro_&]:bg-[#2A273F] text-teal-500 in-[.modo-oscuro_&]:text-[#A394D6] w-9 h-9 rounded-full font-bold flex items-center justify-center cursor-pointer shadow-md border border-rose-100 in-[.modo-oscuro_&]:border-[#433D60] active:scale-90" title="Copiar Outfit">⎘</button>
                <button onClick={() => solicitarEliminacionConjunto(conj.id)} className="bg-white in-[.modo-oscuro_&]:bg-[#2A273F] text-red-500 w-9 h-9 rounded-full font-bold flex items-center justify-center cursor-pointer shadow-md border border-rose-100 in-[.modo-oscuro_&]:border-[#433D60] active:scale-90" title="Eliminar Outfit">✕</button>
              </div>

              <h3 className="font-extrabold text-xl mb-4 text-slate-800 in-[.modo-oscuro_&]:text-[#E0D8F0] pl-1 pr-20 truncate">{conj.nombre}</h3>
      ...
      {conjuntoADuplicar && (
        <div className="fixed inset-0 bg-slate-900/40 flex items-center justify-center z-100 p-4 backdrop-blur-sm">
          <form onSubmit={ejecutarDuplicado} className="bg-white in-[.modo-oscuro_&]:bg-[#1F1D2B] border border-rose-100 in-[.modo-oscuro_&]:border-[#433D60] rounded-2xl shadow-2xl p-5 max-w-sm w-full">
            <h3 className="text-lg font-bold text-slate-800 in-[.modo-oscuro_&]:text-[#E0D8F0] mb-4">Duplicar Outfit</h3>
            <div className="mb-3">
              <label className="block text-sm font-bold text-slate-700 in-[.modo-oscuro_&]:text-[#D1C4E9] mb-1">Nueva Maleta:</label>
              <select name="maletaId" required className="w-full p-3 rounded-xl bg-rose-50 in-[.modo-oscuro_&]:bg-[#2A273F] border border-rose-200 in-[.modo-oscuro_&]:border-[#433D60] text-slate-800 in-[.modo-oscuro_&]:text-[#E0D8F0] outline-none">
                {maletas.map(m => <option key={m.id} value={m.id}>{m.nombre}</option>)}
              </select>
            </div>
            <div className="mb-5">
              <label className="block text-sm font-bold text-slate-700 in-[.modo-oscuro_&]:text-[#D1C4E9] mb-1">Nombre de la copia:</label>
              <input type="text" name="nombreCopia" autoComplete="off" defaultValue={`${conjuntoADuplicar.nombre} (Copia)`} required className="w-full p-3 rounded-xl bg-white in-[.modo-oscuro_&]:bg-[#13111C] border border-rose-200 in-[.modo-oscuro_&]:border-[#433D60] text-slate-800 in-[.modo-oscuro_&]:text-[#E0D8F0] outline-none" />
            </div>
            <div className="flex gap-3 justify-center">
              <button type="submit" className="bg-teal-400 in-[.modo-oscuro_&]:bg-[#7E67C9] text-white font-bold py-2.5 px-4 rounded-xl flex-1 active:scale-95 text-sm">Clonar</button>
              <button type="button" onClick={() => setConjuntoADuplicar(null)} className="bg-slate-100 in-[.modo-oscuro_&]:bg-[#2A273F] text-slate-800 in-[.modo-oscuro_&]:text-[#E0D8F0] font-bold py-2.5 px-4 rounded-xl flex-1 active:scale-95 text-sm">Cancelar</button>
            </div>
          </form>
        </div>
      )}
